...
```solidity
function acceptAdmin() external nonpayable returns (bool)
```
...
```solidity
function setPendingAdmin(address newPendingAdmin) external nonpayable returns (bool)
```
...
```solidity
function snapshot() external nonpayable returns (uint256)
```
...
| Name | Type | Description |
|---|---|---|
| oldPendingAdmin `indexed` | address | undefined |
